  ...
  <div>
    <div class="fullscreen-overlay" v-if="showOverlay">
      <div class="loading-wrapper">
        ...
        <div class="loading-text">图片识别中...</div>
      </div>
    </div>
    <el-upload
      ...
      <div class="el-upload__tip" slot="tip">只能上传jpg/jpeg文件</div>
    </el-upload>
  </div>
</template>
  ...
  methods: {
    handleBeforeUpload(file) {
      ...
      if (!allowedTypes.includes(fileType)) {
        this.$message.error('暂时只支持JPG 和 JPEG 格式的文件');
        return false; // 阻止文件上传
      }
    ...
    uploadHttpRequest(param) {
      ...
      this.$axios.postForm('/upload-pic', data, {
        ...
        Temp.setAttribute('download', window.decodeURI(filename));
